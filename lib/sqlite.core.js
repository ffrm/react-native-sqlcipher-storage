...
SQLiteFactory.prototype.openDatabase = argsArray(function(args) {
  ...
  openargs.dblocation = dblocation || dblocations[Object.keys(dblocations)[0]];
...
SQLiteFactory.prototype.deleteDatabase = function(first,success, error) {
  ...
  if (first.constructor === String) {
    args.path = first;
    args.dblocation = dblocations[Object.keys(dblocations)[0]];
  } else {
    if (!(first && first['name'])) {
    ...
    args.dblocation = dblocation || dblocations[Object.keys(dblocations)[0]];
  }
